...
interface ChildProfile {
  ...
  studentUser?: { username: string } | null;
}
...
const tiles = [
  {
    ...
    href: "/dashboard/parent/emotion-history",
  },
  {
    label: "Symptom Tracking",
    icon: SymptomIcon,
    color: "bg-orange-50 text-orange-600",
    href: "/dashboard/parent/symptom-history",
  },
  {
    label: "Study Module",
    icon: StudyIcon,
    color: "bg-blue-50 text-blue-600",
    href: "/dashboard/parent/study-module",
  },
  {
    label: "Reports",
    icon: ReportsIcon,
...
export default function ParentDashboardPage() {
  ...
  useEffect(() => {
    const fetchChild = async () => {
      ...
      try {
        const res = await fetch("http://localhost:5000/api/students/child", {
        ...
        <div className="bg-white rounded-md shadow-sm p-6">
          ...
          {loading ? (
            ...
            <dl className="space-y-3 text-sm">
              <Row label="Name" value={`${child.firstName} ${child.lastName}`} />
              <Row label="Grade" value={child.grade} />
              {child.section && <Row label="Section" value={child.section} />}
              <Row label="Diagnosis" value={child.diagnosis} />
              <Row label="Communication" value={child.communicationLevel} />
              <Row
                label="Shadow Teacher"
                value={child.assignedTeacher?.name || "Unassigned"}
              />
            </dl>
          ) : null}
        </div>
