...
export default function AdminBranchGradePage({
  ...
  params: Promise<{ branchName: string; categorySlug: string; gradeSlug: string }>;
}) {
  const { branchName, categorySlug, gradeSlug } = use(params);
        ...
        if (data.success) {
          setStudents(
            data.students.filter(
              (s: Student) => s.branch === branch && s.grade === grade?.label
            )
          );
        } else {
    ...
    <DashboardLayout>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-md shadow-sm p-6 text-sm text-gray-400">
          No students in {grade.label} at {branch} yet.
        </div>
      ) : (
