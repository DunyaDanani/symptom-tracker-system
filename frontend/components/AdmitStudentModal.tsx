...
interface AdmitStudentModalProps {
  open: boolean;
  onClose: () => void;
  /** Called after a student is successfully admitted (e.g. to refresh a list/stats). */
  onAdmitted?: () => void;
}
...
export default function AdmitStudentModal({
  ...
  const validateStep = (stepNum: number): Record<string, string> => {
    ...
    if (stepNum === 1) {
      ...
      if (!form.fullName.trim())
        errors.fullName = `${REQUIRED_LABELS.fullName} is required`;
      if (!form.dateOfBirth)
      ...
      if (!form.programCategory)
        errors.programCategory = `${REQUIRED_LABELS.programCategory} is required`;
  ...
  const handleConfirm = async () => {
    ...
    if (form.assignedTeacherId) {
      ...
      payload.newTeacher = {
        ...
        phone: form.newTeacher.phone.trim(),
        nic: form.newTeacher.nic.trim(),
      };
    }
  ...
  const resetForm = () => {
    setForm({ ...blankForm(), branch: branches[0] || "" });
    setCredentials({});
    setEmailSent(false);
    setSuccess(false);
    setFieldErrors({});
  ...
  const gradeOptions = getGradesForCategory(form.categorySlug, form.programCategory);

  return (
    <Modal
      open={open}
      onClose={handleClose}
      title={success ? "Student Admitted" : "Admit New Student"}
      maxWidthClassName="max-w-3xl"
    >
      {success ? (
        <div>
          <div className="bg-green-50 border border-green-200 text-green-800 text-sm rounded p-4 mb-6">
            {form.fullName} has been admitted successfully.{" "}
            {emailSent
          ...
          {step === 1 && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="Full Name" error={fieldErrors.fullName} className="col-span-2">
                  <input
                ...
                <Field label="Category" error={fieldErrors.programCategory}>
                  <select
                    className={`input ${fieldErrors.programCategory ? "input-error" : ""}`}
                    value={form.programCategory}
                    onChange={(e) => {
                      const programCategory = e.target.value as ProgramCategory | "";
                      setForm((prev) => ({ ...prev, programCategory, grade: "" }));
                    }}
                  >
                    <option value="">Select</option>
                ...
                <div className="grid grid-cols-2 gap-4 border border-gray-100 rounded p-4">
                  ...
                  <Field label="Phone Number">
                    <input
                      ...
                      value={form.newTeacher.phone}
                      onChange={(e) =>
                        updateNewTeacherField("phone", e.target.value)
                      }
                    />
                  </Field>
                  <Field label="NIC" className="col-span-2">
                    <input
                      className="input"
                      placeholder="e.g. 199012345678 or 901234567V"
                      value={form.newTeacher.nic}
                      onChange={(e) =>
                        updateNewTeacherField("nic", e.target.value)
                      }
                    />
                  </Field>
                </div>
          ...
          {step === 4 && (
            <div>
              ...
              <SummaryRow
                label="Category"
                value={
                  PROGRAM_CATEGORIES.find((c) => c.value === form.programCategory)
                    ?.label || ""
                }
              />
              ...
              <h3 className="font-semibold text-gray-700 border-b border-gray-100 pb-2 mb-4 mt-6">
                Parent Summary
              </h3>
              <SummaryRow
                label="Parent"
              ...
              <p className="text-xs text-gray-400 mt-6">
                Login credentials for the parent will be auto-generated and
                shown after you confirm. The child does not get a login —
                emotion check-ins are recorded by the assigned shadow
                teacher.
              </p>
            </div>
          )}
