...
interface AddPrincipalModalProps {
  open: boolean;
  onClose: () => void;
  /** Called after a principal account is successfully created (e.g. to refresh a list). */
  onCreated?: () => void;
}
...
export default function AddPrincipalModal({
    ...
    const loadBranches = async () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          if (data.branches.length > 0) setBranch((prev) => prev || data.branches[0]);
        }
      } catch (err) {
  ...
  return (
    <Modal
      open={open}
      onClose={handleClose}
      title={success ? "Principal Account Created" : "Create Branch Principal"}
    ...
    >
      {success ? (
        <div>
          ...
          <h3 className="font-semibold text-gray-700 mb-4">Login credentials</h3>
